...
@Component({
  ...
  templateUrl: './new-spt-quiz.html',
  styleUrls: ['./new-spt-quiz.scss'],
})
export class NewSptQuiz implements OnInit, OnDestroy {
  ou_level: number = 3;
  ...
  // Submit function to save form data to current assessment
  onSubmit(): void {
    console.log('quiz form: ', this.quizForm.value);
    const sptSub: ChecklistBase = {
      id: Math.random().toString(36).substring(2, 15), // Generate a random ID
      ou_level: this.ou_level,
      department: this.department,
      health_area: this.health_area,
      sections: this.quizForm.value.sections,
    };
    ...
    this.curAssmtService.currentAssessment.set(sptSub);
    console.log('current assessment: ', this.curAssmtService.currentAssessment());
  }
